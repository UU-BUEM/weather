#!/usr/bin/env python3
"""Multi-year MERRA-2 pipeline: every year in a range.

Mirrors the ERA5-Land ``test_era5_multi_year.py`` flag structure and
delegates each year to ``test_merra2_one_year.py`` in a child process.

Flow
----
::

    test_merra2_multi_year.py
      ├─ ThreadPoolExecutor(max_workers=parallel_years)
      │    └─ For each year in [from_year … to_year]:
      │         subprocess: python test_merra2_one_year.py --year YEAR
      │              └─ Phase 1 — Download (OPeNDAP, daily rad+slv files)
      │              └─ Phase 2 — Transform + Export
      │                  └─ 12 × MERRA2_<YYYY>_<MM>_all_attrs.nc
      └─ Summary: pass/fail per year

MERRA-2 coverage
----------------
MERRA-2 runs from **1980 to present**. Adjust ``--to-year`` as newer
years are released.

.. note::
   Unlike ERA5-Land's CDS queue, GES DISC's OPeNDAP server has no
   per-account job queue, so running multiple years in parallel is more
   viable here — still watch server-side rate limits if raising
   ``--parallel-years``.
...
All years (sequential)::
    ...
    python src/weather/tests/test_merra2_multi_year.py \\
        --from-year 1980 --to-year 2025 --ncores 12
...
Resume an interrupted run::
    ...
    python src/weather/tests/test_merra2_multi_year.py \\
        --from-year 1980 --to-year 2025 --resume

Flags
-----
--from-year YEAR     First year (default: 1980)
...
def run_year(
    ...
    """Run ``test_merra2_one_year.py`` for *year* in a child process."""
    cmd = [
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Multi-year MERRA-2 pipeline — processes every year in "
            "[from_year, to_year] via test_merra2_one_year.py."
        ),
...
def main() -> None:
    ...
    if args.ncores is None:
        try:
            ncores = int(os.environ.get("MERRA_NCORES", "4"))
        except ValueError:
            ncores = 4
    else:
        ncores = args.ncores
    ...
    if args.work_dir:
        ...
        _wd = os.environ.get("MERRA_WORK_DIR")
        if _wd:
            out_dir = Path(_wd) / "output"

    logger.info("=" * 68)
    logger.info("MERRA-2 Multi-Year Pipeline")
    logger.info(
        "  Years          : %d – %d  (%d total)",
