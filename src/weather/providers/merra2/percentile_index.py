"""MERRA-2 percentile indexer - pure CPU/numpy implementation.

Mirrors ``era5_land.percentile_index``/``cosmo_rea6.percentile_index``
(the KS-distance method actually in production for both). Derives P10,
P50, and P90 representative-year mosaics for every grid cell across the
MERRA-2 Europe-crop grid using the Finkelstein-Schafer (FH) KS-distance
method applied to monthly GHI.

Pipeline
--------
1. Load   : monthly NetCDF files (``MERRA2_<YYYY>_<MM>_all_attrs.nc``)
...
- Grid size (``n_y``/``n_lon``) is inferred from the first loaded file,
  same as ERA5-Land — MERRA-2's own native 0.5x0.625 deg grid for the
  configured Europe box (see ``config.py``'s ``area``), not hardcoded.
- Timestamps are ``HH:30``, not ``HH:00`` (see
  ``docs/MERRA2_PIPELINE_GUIDE.md``'s "Timestamp convention" section);
  this has no effect on the daily-sum KS logic here, which only cares
  about calendar day, not hour-of-day alignment.
...
_FILENAME_RE = re.compile(r"MERRA2_(\d{4})_(\d{2})_")
...
def _preprocess_single_file(file_path: str) -> dict:
    ...
    ----------
    file_path : str
        Absolute path to a MERRA-2 NetCDF file.  The filename must
        contain a ``MERRA2_YYYY_MM_`` segment used to extract year
        and month.
...
def _build_month_mosaic(args: tuple) -> str:
    ...
    for _pct in ("p10", "p50", "p90"):
        _lock = os.path.join(
            target_dir,
            f"merra2_{_pct}_{month_str}_all_attrs.nc.lock",
        )
    ...
    expected_files = [
        os.path.join(
            target_dir,
            f"merra2_{pct.lower()}_{month_str}_all_attrs.nc",
        )
    ...
    tmp_root = tempfile.mkdtemp(prefix=f"merra2_m{month_str}_")
    try:
        for pct in ("P10", "P50", "P90"):
            ...
            fname = f"merra2_{pct.lower()}_{month_str}_all_attrs.nc"
...
class Merra2PercentileIndexer:
    """Create monthly P10/P50/P90 mosaics from historical MERRA-2 data.

    The pipeline has three sequential phases:

    1. **Load** (``compile_historical_baselines``): read all input
       NetCDF files (``MERRA2_<YYYY>_<MM>_all_attrs.nc``) in parallel
    ...
    ----------
    source_dir : str
        Directory containing input MERRA-2 NetCDF files.  Files must
        match the pattern ``MERRA2_YYYY_MM_*.nc``.
    target_dir : str
    ...
    n_cpu_cores : int, optional
        ...
        MERRA-2 processing is I/O-bound (OPeNDAP-fetched daily files
        merged into monthly datasets), so this defaults to 8 to match
        ``MERRA_NCORES`` rather than COSMO's 94-core CPU-bound default.
    n_y, n_lon : int, optional
        Grid dimensions.  If ``None`` (default), inferred from the first
        successfully loaded source file — the MERRA-2 Europe-crop grid
        size depends on ``MERRA2_AREA`` and should not be hardcoded.
    ...
    def execute_indexing_pipeline(self) -> None:
        ...
        """
        file_paths = sorted(
            glob.glob(os.path.join(self.source_dir, "MERRA2_*.nc"))
        )
        if not file_paths:
            raise FileNotFoundError(
                f"No MERRA2_*.nc files found in {self.source_dir}"
            )
        ...
        all_outputs = [
            os.path.join(
                self.target_dir,
                f"merra2_{pct.lower()}_{m:02d}_all_attrs.nc",
            )
            for pct in ("P10", "P50", "P90")
            for m in range(1, 13)
        ]
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(
        description="MERRA-2 percentile indexer",
    )
    _parser.add_argument(
        "--source-dir",
        default=None,
        help=(
            "Directory containing monthly MERRA2_*.nc files."
            " Default: EnvSettings.merra2_output_dir()"
        ),
    )
    _parser.add_argument(
        "--target-dir",
    ...
    _source = _args.source_dir or str(EnvSettings.merra2_output_dir())
    ...
    if _args.months:
        for _m in _args.months:
            ...
            for _pct in ("p10", "p50", "p90"):
                _f = os.path.join(
                    _target,
                    f"merra2_{_pct}_{_m:02d}_all_attrs.nc",
                )
    ...
    indexer = Merra2PercentileIndexer(
        source_dir=_source,
        target_dir=_target,
        n_cpu_cores=EnvSettings.merra2_ncores(),
        n_mosaic_workers=2,
    )
