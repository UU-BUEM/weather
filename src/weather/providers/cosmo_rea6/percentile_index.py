...
Modules
-------
_preprocess_single_file    : worker - load one NetCDF file.
_build_month_mosaic        : worker - assemble and write one month.
CosmoRea6PercentileIndexer : orchestrator class.
"""
...
def _preprocess_single_file(file_path: str) -> dict:
    ...
    ----------
    file_path : str
        Absolute path to a COSMO-REA6 NetCDF file.  The filename must
        contain a ``_YYYY_MM_`` segment used to extract year and month.
...
def _build_month_mosaic(args: tuple) -> str:
    ...
    Outer loop  : years (24 iterations, one file open per iteration)
    ...
    for _pct in ("p10", "p50", "p90"):
        _lock = os.path.join(
            target_dir,
            f"cosmo_rea6_{_pct}_{month_str}_all_attrs.nc.lock",
        )
    ...
    expected_files = [
        os.path.join(
            target_dir,
            f"cosmo_rea6_{pct.lower()}_{month_str}_all_attrs.nc",
        )
    ...
    # For each year we open the source file exactly once, load all
    # variables into memory, then scatter-write winning cells into
    # every percentile mosaic.  This is correct because:
    #
    #   "Once P50 for January is determined to be year 2014, all
    #    other variables for year 2014 at those cells are attached."
    #
    # The variable loop is the innermost loop (cheapest), not the
    # file-open loop.
    for local_yi, year in enumerate(sorted_years):
        ...
        with xr.open_dataset(fp, engine="netcdf4") as ds:
            ...
            # Load all variables at once before closing the file.
            # Guard ndim again in case a year's file has extra dims.
    ...
    # ── Write output files ──────────────────────────────────────────
    # Write to a local /tmp directory first to avoid NFS locking.
    # HDF5_USE_FILE_LOCKING=FALSE suppresses HDF5 internal locks
    # but NOT the POSIX advisory locks that NFS enforces at the OS
    # level.  Writing to local disk has no such locks; once the
    # file is complete we move it to the NFS target atomically.
    # Write each percentile file to local disk first, then move to
    # the (NFS) target.  Local writes avoid both NFS POSIX locks and
    # the HDF5 contention seen when many workers write to NFS at once.
    os.makedirs(target_dir, exist_ok=True)
    saved: list[str] = []

    tmp_root = tempfile.mkdtemp(prefix=f"cosmo_m{month_str}_")
    try:
        for pct in ("P10", "P50", "P90"):
            ...
            fname = (
                f"cosmo_rea6_{pct.lower()}"
                f"_{month_str}_all_attrs.nc"
            )
            tmp_path = os.path.join(tmp_root, fname)
            # Explicit engine + format avoids HDF5 auto-detect quirks
            out_ds.to_netcdf(
                tmp_path,
                encoding=enc,
                engine="netcdf4",
...
class CosmoRea6PercentileIndexer:
    """Create monthly P10/P50/P90 mosaics from historical COSMO-REA6 data.
    ...
    ----------
    source_dir : str
        Directory containing input COSMO-REA6 NetCDF files.  Files must
        match the pattern ``*_YYYY_MM_*.nc``.
    target_dir : str
        Directory where output percentile NetCDF files are written.
    n_cpu_cores : int, optional
        Number of parallel workers for the file-loading phase.
        Default: 94.
    n_y : int, optional
        Number of grid rows (y dimension).  Default: 824.
    n_lon : int, optional
        Number of longitude grid points.  Default: 848.
    n_mosaic_workers : int, optional
        Number of parallel workers for the mosaic-writing phase.
        Each worker holds approximately 8 GB RAM peak; set this so
        that ``n_mosaic_workers * 8 GB`` stays within available RAM.
        Default: 6.
    """
    ...
    def _compute_ks_for_month(
        ...
        Runtime is under one second per month.
    ...
    def construct_and_save_mosaics(
        ...
        This phase is I/O-bound: each month reads up to 24 source files
        (~4 GB each).  Keep ``n_mosaic_workers`` small (1-2) so the
        workers do not saturate NFS bandwidth and stall each other.
        There is no per-job timeout; every month runs to completion.
        ...
        # No timeout.  The mosaic phase is I/O-bound: each month reads
        # up to 24 source files (~4 GB each) from NFS.  Under shared
        # bandwidth a single month can legitimately take 30-60 min, and
        # months waiting in the queue behind a busy worker take even
        # longer in wall-clock terms.  A submission-time deadline would
        # wrongly kill months that simply have not started yet.  We let
        # every month run to completion and only report genuine errors.
    ...
    def execute_indexing_pipeline(self) -> None:
        ...
        """
        file_paths = sorted(
            glob.glob(os.path.join(self.source_dir, "*.nc"))
        )
        if not file_paths:
            raise FileNotFoundError(
                f"No .nc files found in {self.source_dir}"
            )

        # Check upfront whether all 36 output files exist and are
        # valid.  If so, skip the entire load + KS phases (~8 min)
        # and go straight to the mosaic phase, which will find each
        # month already done and return immediately.
        all_outputs = [
            os.path.join(
                self.target_dir,
                f"cosmo_rea6_{pct.lower()}_{m:02d}_all_attrs.nc",
            )
            for pct in ("P10", "P50", "P90")
            for m in range(1, 13)
        ]
        ...
        for fp in file_paths:
            fname_match = re.search(
                r"_(\d{4})_(\d{2})_", os.path.basename(fp)
            )
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(
        description="COSMO-REA6 percentile indexer",
    )
    ...
    # basicConfig called exactly once here with INFO level.
    # The CET Formatter.converter is already set at module level.
    logging.basicConfig(
        level=logging.INFO,
    ...
    _target = "/data/soma/cosmo_rea6/output/percentile"
    ...
    if _args.months:
        for _m in _args.months:
            ...
            for _pct in ("p10", "p50", "p90"):
                _f = os.path.join(
                    _target,
                    f"cosmo_rea6_{_pct}_{_m:02d}_all_attrs.nc",
                )
    ...
    # These accumulate after interrupted runs and cause silent hangs.
    _source = "/data/soma/cosmo_rea6/output"
    ...
    indexer = CosmoRea6PercentileIndexer(
        source_dir=_source,
        target_dir=_target,
        n_cpu_cores=94,
        n_mosaic_workers=2,
